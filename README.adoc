...
public class ExcelModuleDemoToDoItemRowHandler implements ExcelFixtureRowHandler {          // <1>
    ...                                                                                     // <2>
    @Override
    public List<Object> handleRow(final FixtureScript.ExecutionContext executionContext) {  // <3>
        final ExcelModuleDemoToDoItem toDoItem = ...;
        return Collections.<Object>singletonList(toDoItem);                                 // <4>
    }
    ..                                                                                      // <5>
}
----
...
<3> `ExecutionContext` can be used to pass parameters down to the row handler
<4> return a list of objects instantiated by this row handler.
<5> eg inject domain services/repositories to delegate to for instantiating objects
...
* `1.10.0-SNAPSHOT` - released against Isis 1.10.0; includes `ExcelFixture`
