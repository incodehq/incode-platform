...
The `render(...)` methods will return either the rendered text, or `null` if could not locate any `DocFragment`.
...
public interface ApplicationTenancyService {
    String atPathFor(final Object domainObjectToRender);    // <1>
