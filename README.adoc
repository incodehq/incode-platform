...
== What happens during bootstrapping
:link-v1: link:https:///blob/master/webapp/src/main/resources/db/migration/V1__schema-simple.sql
:link-v2: link:https:///blob/master/webapp/src/main/resources/db/migration/V2__table-simple_SimpleObject.sql
...
** as a minor refinement, all that is required is the schema of this database.
You could therefore have a CI pipeline that hooks onto your nightly database backups; this would restore the production database, then truncate all tables, then creates a backup of that truncated database. +
+
See link:https:///blob/master/scripts/sqlserver/truncate-all-tables.sql[truncate-all-tables.sql] for a script that does this for MS SQL Server.
