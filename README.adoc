...
* `1.14.6` - fixes https://github.com/incodehq/incode-module-document/issues/21[#21] (remove attachments), https://github.com/incodehq/incode-module-document/issues/22[#22] (`PaperclipForDocument.attachedToId` column).
+
[NOTE]
====
This release requires a database migration: the `PaperclipForDocument` table's `communicationId` column has been renamed (corrected) to be called `attachedToId`.
====
...
[source]
----
sh interim-release.sh 1.14.6 origin
----

where

* `1.14.6` is the base release
...
[source]
----
git push origin master && git push origin 1.14.6
----
