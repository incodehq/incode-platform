...
Then, we can reconfigure the app to use these different ports by editing link:blob/master/webapp/src/main/resources/spring.properties[`spring.properties`]:
...
+
In the example app this is done using Spring (link:blob/master/webapp/src/main/resources/activemq-config.xml[activemq-config.xml]):
+
[source,xml]
----
<beans
