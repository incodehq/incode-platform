...
In order to be able to add/remove aliases to a domain object, you need to implement a subclass of `AliasLink` for
that domain object type.  This link acts as a type-safe tuple linking domain object to `Alias`.  For each such
"aliased" domain object, the module contributes the `aliases` collection, and also contributes the `addAlias(...)`
and `removeAlias(...)` actions.

Specifically, subclass the `AliasLink` abstract class and also implement a domain event subscriber to correctly
instantiate the subclass.

For example, in the demo app the `AliasLinkForDemoObject` implements both of these responsibilities:
...
* `1.13.0` - released against Isis 1.13.0; removing the `Aliasable` interface (completely decouples the "aliased" domain object from this module.
