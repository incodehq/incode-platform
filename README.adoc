...
** `autoCreateAll=false` re-enables FlywayDB, causing it apply the migration scripts
...
If you look at the implementation you'll see that the `delta.sh` script uses a link:/util/scripts/schema.txt[schema.txt] file which lists all of the database schemas to compare.
