...
----
<1> initial page to render, defaults to first page
<2> scaling, defaults to 100%
<3> height of the panel, in pixels
...
Related, if a user were to view a first document and navigate to a page 2 or 3, then move to second document, and then back to the first, it would again be annoying if the user had start back at page 1 and navigate once more to a different page.

To support these two use cases the component therefore provides an optional SPI service as a plugin point.
