...
Once a `Document` has been created, other supporting pre-existing ``Document``s can then be attached to it.
For example, these could be tax receipts for an invoice.
These supporting ``Document``s do require a `DocumentType` to help categorise them, but no corresponding `DocumentTemplate`s are required because they are never rendered (they exist already).
An SPI service (`DocumentAttachmentAdvisor`) is used to obtain the list of available ``DocumentType``s to use.

Currently, only PDF ``Document``s can be so attached.
...
The idea is that this will - in a new branch - update the `dom/pom.xml` with a timestamped version (eg `1.14.2.20170331-0740`).
