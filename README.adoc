...
Now if any of the tables are to be managed in a default schema (such as "dbo" for SQL Server, or "PUBLIC" for HSQLDB), then either:

* do not list that default schema within `flyway.schemas`, and do _not_ provide migration scripts to create the schemas themselves

* simpler, simply list "flyway" as the one and only schema in `flyway.schemas`, and then _do_ provide a migration script. +
+
This will have the result of creating a new `flyway.schema_version` table.

Of these two options, the demo application for this module goes with the second approach.
