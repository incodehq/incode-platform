= incode-module-docrendering-freemarker
:_imagesdir: ./

image:https://travis-ci.org/incodehq/incode-module-docrendering-freemarker.png?branch=master[Build Status,link=https://travis-ci.org/incodehq/incode-module-docrendering-freemarker]

This module, intended for use with link:http://isis.apache.org[Apache Isis], provides an implementation of the Incode http://github.com/incodehq/incode-module-document[Document] module's `Renderer` interface using the (non-ASF) http://github.com/isisaddons/isis-module-freemarker[Isis addons' freemarker] module.


== API

The module provides the `RendererForFreemarker` class that implements `RendererFromCharsToChars`:
...
This class can be used as the `Renderer` implementation for a Document `RenderingStrategy`.
The `RenderingStrategyFSAbstract` fixture script can be used to create such an entity, eg:
