...
You could therefore have a CI pipeline that hooks onto your nightly database backups; this would restore the production database, then truncate all tables, then creates a backup of that truncated database.  See link:/util/scripts/truncate-all-tables.sql[truncate-all-tables.sql] for a script that does this for MS SQL Server.
