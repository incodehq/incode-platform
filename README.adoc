...
communication channel objects (postal address, email or phone/fax number) to domain objects.  The only requirement is
for the domain object to implement the `CommunicationChannelOwner` marker interface.
...
Each communication channel object can be categorized, eg "work email", "home phone number".  The list of
defined by an optional SPI service.
