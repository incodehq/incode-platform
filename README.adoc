...
image:https://travis-ci.org/incodehq/incode-module-fixturesupport.png?branch=master[Build Status,link=https://travis-ci.org/incodehq/incode-module-fixturesupport]

This module, intended for use with link:http://isis.apache.org[Apache Isis], provides support for writing fixtures.
...
public interface DemoData<D extends Enum<D>, T> {
    T asDomainObject();
    T persistWith(RepositoryService repositoryService);
    T findWith(RepositoryService repositoryService);
}
----
...
public enum DemoInvoiceData implements DemoData<DemoInvoiceData, DemoInvoice> {
    ...
    @Programmatic
    public DemoInvoice persistWith(RepositoryService repositoryService) {
        return Util.persist(this, repositoryService);                       // <4>
    }

    @Programmatic
    public DemoInvoice findWith(RepositoryService repositoryService) {
        return Util.firstMatch(this, repositoryService);                    // <5>
    }
    ...
}
----
