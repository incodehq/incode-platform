...
public class WicketChartsDemoToDoItem_recreate5_for extends FixtureScript {

    private final String user;

    public WicketChartsDemoToDoItem_recreate5_for() {
        this(null);
    }
    
    public WicketChartsDemoToDoItem_recreate5_for(String ownedBy) {
        this.user = ownedBy;
    }
    ...
    protected void execute(ExecutionContext executionContext) {
        ...
        executionContext.executeChild(this, new WicketChartsDemoToDoItem_tearDown2(ownedBy));
    ...
    private void installFor(String user, ExecutionContext executionContext) {
        ...
        WicketChartsDemoToDoItem t1 = createToDoItemForUser("Buy milk", Category.Domestic, Subcategory.Shopping, user, daysFromToday(0), new BigDecimal("0.75"), executionContext);
        WicketChartsDemoToDoItem t2 = createToDoItemForUser("Buy bread", Category.Domestic, Subcategory.Shopping, user, daysFromToday(0), new BigDecimal("1.75"), executionContext);
        WicketChartsDemoToDoItem t3 = createToDoItemForUser("Buy stamps", Category.Domestic, Subcategory.Shopping, user, daysFromToday(0), new BigDecimal("10.00"), executionContext);
        t3.setComplete(true);
        WicketChartsDemoToDoItem t4 = createToDoItemForUser("Pick up laundry", Category.Domestic, Subcategory.Chores, user, daysFromToday(6), new BigDecimal("7.50"), executionContext);
        WicketChartsDemoToDoItem t5 = createToDoItemForUser("Mow lawn", Category.Domestic, Subcategory.Garden, user, daysFromToday(6), null, executionContext);
    ...
    private WicketChartsDemoToDoItem createToDoItemForUser(final String description, final Category category, Subcategory subcategory, String user, final LocalDate dueBy, final BigDecimal cost, ExecutionContext executionContext) {
        final WicketChartsDemoToDoItem toDoItem = demoToDoItemMenu.newToDoItem(description, category, subcategory, user, dueBy, cost);
