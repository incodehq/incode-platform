...
@javax.jdo.annotations.Queries( {
    @javax.jdo.annotations.Query(
            ...
            value = "SELECT "
                    + "FROM org.isisaddons.wicket.wickedcharts.fixture.demoapp.todomodule.dom.WickedChartsDemoToDoItem "
...
@DomainObject(
        autoCompleteRepository = WickedChartsDemoToDoItemMenu.class
)
@DomainObjectLayout(
        named = "To Do Item",
...
public class WickedChartsDemoToDoItem implements Comparable<WickedChartsDemoToDoItem>, CalendarEventable, Locatable {
    ...
    public String iconName() {
        return "WicketChartsModuleDemoToDoItem-" + (!isComplete() ? "todo" : "done");
    }
    ...
    @Getter @Setter
    @javax.jdo.annotations.Persistent(table="WickedChartsDemoToDoItemDependencies")
    @javax.jdo.annotations.Join(column="dependingId")
    @javax.jdo.annotations.Element(column="dependentId")
    @CollectionLayout(sortedBy = DependenciesComparator.class)
    private SortedSet<WickedChartsDemoToDoItem> dependencies = new TreeSet<>();
    ...
    public WickedChartsDemoToDoItem completed() {
        setComplete(true);
        return this;
    }
    ...
    public WickedChartsDemoToDoItem notYetCompleted() {
        setComplete(false);
    ...
    public WickedChartsDemoToDoItem updateCosts(
            @Nullable
            @javax.validation.constraints.Digits(integer=10, fraction=2)
    ...
    public WickedChartsDemoToDoItem add(
            final WickedChartsDemoToDoItem toDoItem) {
        getDependencies().add(toDoItem);
        return this;
    }
    public List<WickedChartsDemoToDoItem> autoComplete0Add(final @MinLength(2) String search) {
        final List<WickedChartsDemoToDoItem> list = toDoItems.autoComplete(search);
    ...
    public String validateAdd(final WickedChartsDemoToDoItem toDoItem) {
    ...
    public WickedChartsDemoToDoItem remove(
            final WickedChartsDemoToDoItem toDoItem) {
    ...
    // validate the provided argument prior to invoking action
    public String validateRemove(final WickedChartsDemoToDoItem toDoItem) {
        if(!getDependencies().contains(toDoItem)) {
    ...
    // provide a drop-down
    public SortedSet<WickedChartsDemoToDoItem> choices0Remove() {
    ...
    public List<WickedChartsDemoToDoItem> delete() {
    ...
    public static class Predicates {
        ...
        public static Predicate<WickedChartsDemoToDoItem> thoseOwnedBy(final String currentUser) {
            return toDoItem -> Objects.equal(toDoItem.getOwnedBy(), currentUser);
        }

        public static Predicate<WickedChartsDemoToDoItem> thoseCompleted(
                final boolean completed) {
            return t -> Objects.equal(t.isComplete(), completed);
        }

        public static Predicate<WickedChartsDemoToDoItem> thoseCategorised(final Category category) {
            return toDoItem -> Objects.equal(toDoItem.getCategory(), category);
        }

        public static Predicate<WickedChartsDemoToDoItem> thoseSubcategorised(
                final Subcategory subcategory) {
            return t -> Objects.equal(t.getSubcategory(), subcategory);
        }

        public static Predicate<WickedChartsDemoToDoItem> thoseCategorised(
    ...
    @Override
    public int compareTo(final WickedChartsDemoToDoItem other) {
    ...
    @javax.inject.Inject
    WickedChartsDemoToDoItemMenu toDoItems;
    //endregion
    ...
    public WickedChartsDemoToDoItem updateLocation(final Double longitude, final Double latitude) {
