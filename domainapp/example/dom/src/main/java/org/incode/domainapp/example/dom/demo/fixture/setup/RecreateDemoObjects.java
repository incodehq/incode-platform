...
public class RecreateDemoObjects extends FixtureScript {
    ...
    protected void execute(final ExecutionContext ec) {
        ...
        // execute
        ec.executeChild(this, new DemoObjectTearDown());
        for (int i = 0; i < number; i++) {
            final String name = NAMES.get(i);
            final DemoObjectMenu_create fs = new DemoObjectMenu_create().setName(name);
            ec.executeChild(this, fs.getName(), fs);
            demoObjects.add(fs.getFlywayDemoObject());
        }
    }
