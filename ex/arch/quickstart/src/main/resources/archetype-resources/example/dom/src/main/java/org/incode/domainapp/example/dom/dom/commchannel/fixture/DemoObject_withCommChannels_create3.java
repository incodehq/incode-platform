...
public class DemoObject_withCommChannels_create3 extends FixtureScript {
	...
	executionContext.executeChild(this, new DemoObject_withCommChannels_tearDown());
        ...
        wrap(addEmailAddress(demoOwner)).${symbol_dollar}${symbol_dollar}("foo@example.com", "Other Email", null);
        wrap(addPhoneOrFaxNumber(demoOwner)).${symbol_dollar}${symbol_dollar}(CommunicationChannelType.PHONE_NUMBER, "555 1234", "Home Number", null);
        wrap(addPhoneOrFaxNumber(demoOwner)).${symbol_dollar}${symbol_dollar}(CommunicationChannelType.FAX_NUMBER, "555 4321", "Work Fax", null);

        final DemoObject bar = create("Bar", executionContext);
        wrap(addPostalAddress(demoOwner)).${symbol_dollar}${symbol_dollar}(
